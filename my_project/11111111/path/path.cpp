...
namespace Path {
    ...
    std::vector<Sector> findSectors(const Map::mapType& _map, const Point _endPoint, SubDivLevel searchLevel) {
        ...
        const double maxRayDistance = 100.0;  // 最大射线距离
        ...
        const double minObstacleDistance = 20.0;  // 关心的障碍物距离范围

        // 找出在关心范围内的障碍物角度
        std::vector<int> obstacleAngles;
        for (const auto& ray : obstacleRays) {
            if (ray.hitObstacle && ray.distance <= minObstacleDistance) {
    ...
    std::vector<Point> getTargetPositions(const Map::mapType& _map, const Point _end,
        ...
        // 分析障碍物并找出清晰扇区
        auto clearSectors = findSectors(_map, _end, searchLevel);
    ...
    std::vector<Point> getNeighbors(Point pos, const Map::mapType& _map, Point start, Point end,
        ...
        auto clearSectors = findSectors(_map, end, searchLevel);
    ...
    std::vector<Point> aStarSearch(Point start, Point target, const Map::mapType& _map,
                                  Point end, int carRadius, int fireRadius, SubDivLevel searchLevel) {
